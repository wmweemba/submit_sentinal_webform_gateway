...
// Reply-To should be a monitored mailbox (not the noreply sending address) so
// recipient mail servers see a valid reply path, and recipients can actually reply.
const SMTP_REPLY_TO = process.env.SMTP_REPLY_TO || SMTP_FROM.match(/<(.+)>/)?.[1] || SMTP_FROM;
...
/**
 * Formats form data into a plain-text email body.
 * Mail providers and spam filters expect a multipart/alternative message
 * with both HTML and text parts; HTML-only emails are a deliverability red flag.
 */
const formatEmailTextContent = (formData, files = []) => {
  const entries = Object.entries(formData)
  ...
  const fileList = files.length > 0
    ? `Attachments (${files.length}):\n${files.map(file => `- ${sanitizeInput(file.originalname)} (${sanitizeInput(file.mimetype)})`).join('\n')}`
    : 'No files were attached to this submission.';

  return `New Form Submission

A new form has been submitted via Submit Sentinel.
...
---
This email was automatically generated by Submit Sentinel.
Submission time: ${new Date().toISOString()}
Powered by Nexus Consulting Services (https://mynexusgroup.com)
`;
};
...
app.post('/submit/:clientSlug', dynamicCors, fileUploadMiddleware, async (req, res) => {
  try {
    ...
    const emailTextContent = formatEmailTextContent(sanitizedBody, allFiles);
